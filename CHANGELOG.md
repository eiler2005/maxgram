...
### Changed
- **Forward source labels** — Telegram now shows `↪️ Переслано из «…»` for MAX forwards when the source chat/channel title is already available in the local MAX cache. The lookup is cache-only; missing or inaccessible sources retain the neutral `↪️ Переслано из MAX` label. No separate title record or log is written; the existing short-lived plaintext text-retry queue retains the rendered marker only when Telegram delivery itself is retried.
