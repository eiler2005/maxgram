...
### Fixed
- **Deferred MAX files from recovery cache** — cache-only `FILE` attachments now download from their encrypted signed URL hint first and retain the existing safe `fileId` fallback; if the embedded source coordinates do not resolve it retries against the receiving wrapper message, so forwarded files without a stable `fileId` can be delivered after the initial attempt.
