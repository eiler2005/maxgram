...
### Added
- **Watchdog summary schedule and `/watchdog` command** — the status summary now arrives four times a day (09:00, 13:00, 17:00, 21:00 MSK) instead of once, which also cuts the time it takes to notice a dead observer from a day to about four hours. A `/watchdog` command describes the observer — layers, what it catches, when it writes, and that it repairs nothing — and offers a "check now" button that requests a fresh summary over the same signed channel the push already uses, adding no new secret, port or firewall rule. The command is handled by the bridge, so it stops working when the bridge is dead; that is the case the observer exists for, and it is documented as a convenience rather than a safety mechanism.
- **External watchdog on a second VPS** — an observer container now watches the bridge from outside its failure domain and reports to owner DM and the ops topic under a header naming the source, so an external alert is never confused with one the bridge sent about itself. It covers the three classes nothing could observe before: a stopped container (`restart: always` does not apply to an explicit stop), a dead host, and a broken Telegram alert path of the bridge itself. Rules use hysteresis, cascade suppression, a dedup window and one-shot recovery notices; each rule names the failure class it covers. The observer only reports — its SSH key is pinned to a read-only probe. Every alert names the observation layer that caught it and the command to start from, and the summary reports all four layers with open problems filed under the layer that found them.
...
### Changed
- **Watchdog scope documentation** — public READMEs, architecture, operator runbook, and project context now distinguish the in-container supervisor/MAX watchdog, Docker restart policy on the Hetzner VPS, and observational healthcheck. They also document that no host-level systemd watcher exists yet, so an explicit Compose stop/down requires an explicit recovery start.
- **Forward source labels** — Telegram now shows `↪️ Переслано из «…»` from PyMax `ForwardLink.chat_name`, with the local MAX cache as a fallback. Missing or inaccessible sources retain the neutral `↪️ Переслано из MAX` label. No live lookup, separate title record, or log is written; the existing short-lived plaintext text-retry queue retains the rendered marker only when Telegram delivery itself is retried.
