...
- **Important-only recovery alerts** — автоматические scans уведомляют owner/ops только о значимых изменениях: новый/unmapped чат, нужен invite/admin или обнаружен новый MAX account; текст содержит только агрегаты и ссылку на `/recovery report`
...
Обычный режим: bridge собирает recovery snapshot после успешного MAX connect/reconnect и дополнительно раз в неделю. Также есть event-driven обновление: новый `ChatBinding`, переименование fallback-title и MAX `CONTROL` событие ставят debounced background scan в `BridgeCore`. Эти scans выполняются асинхронно через task scheduler и не задерживают пересылку сообщений, создание topics или обычный routing. Если автоматический scan нашёл новый/unmapped чат, состояние `needs_invite`/`manual_admin_required` или новый MAX account, owner/ops получает короткое important-only уведомление с агрегатами и подсказкой открыть `/recovery report`.
...
Правило приватности: registry хранит только recovery metadata. Тексты сообщений, media URLs, signed tokens и raw MAX payload не сохраняются. Group-visible reports, logs, health state и automatic notifications не содержат invite links, manual notes, phone numbers, message text или raw MAX fields. `/recovery export` может содержать invite links и admin notes, поэтому отправляется только владельцу в DM.
