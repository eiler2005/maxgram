...
- **Явная adapter/backend boundary** — `BridgeCore` зависит от transport-neutral contracts; `MaxAdapter` стал facade над operation services, а `pymax` изолирован в `src/adapters/max/backends/pymax/`; это защищено regression-тестами
