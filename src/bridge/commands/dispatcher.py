...
from . import watchdog as bridge_watchdog_command
...
class BridgeCommandDispatcher:
    ...
    def register(self):
        ...
        self._tg.on_command("watchdog", self.handle_watchdog)
    ...
    async def handle_watchdog(self):
        """Описание внешнего наблюдателя плюс кнопка «проверить сейчас»."""
        return bridge_watchdog_command.build_message(), bridge_watchdog_command.build_buttons()
