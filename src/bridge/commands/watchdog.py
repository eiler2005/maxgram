...
#: Публичный репозиторий проекта — тот же, что в бейджах README.
DOCS_URL = "https://github.com/eiler2005/maxgram/blob/master/docs/runbooks/watchdog.md"
...
def build_message() -> str:
    ...
    Только plain text: ответы команд уходят через message.reply() без parse_mode,
    поэтому HTML-теги приехали бы в чат как текст. Структуру держим на эмодзи и
    отступах, как в /help.
    """
    lines = [
        "🛰 Внешний watchdog",
        "",
        "Наблюдает за этим bridge с ОТДЕЛЬНОГО VPS — другой провайдер,",
        ...
        "",
        "🎯 Что он ловит",
        "  Остановленный контейнер, мёртвый хост и сломанную доставку",
        "  алертов — то, о чём bridge не может сообщить сам.",
        "",
        "🕘 Когда пишет",
        "  Сводка 4 раза в сутки: 09:00, 13:00, 17:00, 21:00 МСК.",
        "  Плюс алерты при проблемах. Проверки идут каждые 60 секунд, но молча.",
        "",
        "🚫 Что он не делает",
