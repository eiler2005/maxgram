...
- `backends/pymax/` — `PymaxBackend` + тонкий `PymaxClientAdapter`; единственное место с `pymax` imports. Внутри пакет разделён на `client_factory.py`, `session_store.py`, `transport.py`, `events.py`, `raw_gateway.py`, `models.py`, `media.py`: PyMax 2 `Client + ExtraConfig`, one-time import legacy PyMax 1 `auth` session into PyMax 2 `sessions`, custom MAX egress transport, native `on_raw`, raw requests через isolated gateway, payload/file construction and conversion into our DTO.
