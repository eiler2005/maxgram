...
3. Посмотреть `/recovery report`: `unmapped` — MAX чаты, видимые новому аккаунту, но ещё не привязанные к старым Telegram topics; `есть invite link` / `нужен админ` — ручные шаги доступа; `DM contacts` / `needs contact/remap` — личные собеседники, которых нужно найти/написать первым сообщением или вручную привязать к старому topic. Для названий, MAX ids и invite/admin/DM деталей используй `/recovery export`, который уходит владельцу в DM.
