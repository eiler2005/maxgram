...
7. Каждые 120 секунд `bridge.dm_history_sweep.worker_started` перечитывает последние 30 сообщений активных DM за окно 48 часов и досылает пропущенные сообщения через обычный dedup path. При успехе ищи `max.history_sweep.replayed` и `tg.outbound.sent media_type=voice`. Если в логах PyMax появляется `unhashable type: 'list'` на `CHAT_HISTORY`, это regression в backend msgpack guard (`BridgeMsgpackPayloadCodec`), а не пользовательская кодировка сообщения. Если появляется `B format requires 0 <= number <= 255` или `pymax_tcp_sequence_overflow`, это regression в backend TCP seq guard (`BridgeConnectionManager`).
