...
- `dm_contact_recovery_registry` — DM-only контакты для восстановления: `max_user_id`, display name, старый/текущий DM chat id, связанный Telegram topic, source, recovery status и `last_scan_at`. Источник — только реальные `client.dialogs` и уже привязанные DM topics; полная MAX address book (`client.contacts`) и `known_users` сюда не копируются.
