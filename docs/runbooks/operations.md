...
- `/recovery report` показывает `Свежесть snapshot` по `last_scan_at`, а также агрегаты `DM contacts`, `linked topics`, `needs contact/remap`; это source of truth для freshness;
