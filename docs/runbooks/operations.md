...
- encrypted payload содержит только поля phone/display-name, которые нужны PyMax `ContactInfo` для `import_contacts()`;
...
- контакты, для которых PyMax не отдаёт `User.phone`, не могут быть автоматически импортированы и остаются manual recovery cases.
