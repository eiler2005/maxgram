...
```
L1:  наблюдатель ──HTTP──► status API bridge      (спрашиваем, что болит внутри)
L2:  наблюдатель ──SSH──► production              (спрашиваем, жив ли контейнер и хост)
L3:  production ──HMAC POST──► наблюдатель        (production рассказывает сам)
L4:  host-мониторинг ──► контейнер наблюдателя    (кто сторожит сторожа)
```
