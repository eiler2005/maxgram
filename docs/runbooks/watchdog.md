...
| **F14** | **Умер сам наблюдатель** | контейнер снят, второй VPS лёг | — | `vps-monitor` на его хосте + встречная проба + сводка ×4 в сутки | ≤5 мин / до 4 ч | вручную |
...
flowchart LR
    ...
    L4["<b>L4 · мета-мониторинг</b><br/>vps-monitor · встречная проба<br/>сводка ×4 в сутки"]
...
| Сводка «наблюдатель жив» ×4 в сутки | этот watchdog | тишину в канале (до 4 часов) |
...
Четыре раза в сутки (09:00, 13:00, 17:00, 21:00 МСК) приходит сводка. Она не просто перечисляет слои — говорит, что
...
🔵 Сводка состояния: наблюдатель на связи
Наблюдаемый хост: maxtg-bridge-prod
...
> **Грабли.** Кнопка работает только если polling запрошен с `callback_query`.
> Bridge долго ходил с `allowed_updates=["message"]`, из-за чего Telegram не
> присылал нажатия вообще — молча не работала и эта кнопка, и более старая
> «Вступить в MAX» для инвайтов. Симптом обманчив: сообщение с кнопкой
> отправляется, кнопка нажимается, и ничего не происходит — ни ошибки, ни
> записи в логах, потому что событие до bridge просто не доезжает.
> Закреплено тестом `test_polling_allows_callback_updates`.
...
### Внешний watchdog промолчал, а bridge прислал алерт

Это штатное поведение, а не пропуск. Внутренние алерты bridge шлёт сразу, а
внешний слой ждёт подтверждения: `subsystem_issue` требует двух подряд опросов
status API, то есть при опросе раз в 300 с — около **10 минут** непрерывной
деградации. Самовосстанавливающийся сбой короче этого порога внешний слой
намеренно не дублирует.

Проверить, что наблюдатель проблему **видел**:

```bash
ssh deploy@<observer_ip> \
  'cd /opt/maxtg-watchdog && docker compose logs --since 30m watchdog | grep "ниже порога"'
```

Строка вида `замечено, ниже порога: subsystem_issue (1/2 подряд)` означает, что
правило сработало и ждёт подтверждения. Её отсутствие при наличии проблемы —
уже повод разбираться: значит данные до наблюдателя не дошли.

> Разбор реального случая 2026-09-10: `home_ru_proxy` отвалился на 4.5 минуты,
> bridge сам прислал degraded и recovery, внешний слой промолчал — правильно.
> Но тогдашний лог писал `all checks passed` и в момент, когда правило уже
> сработало, поэтому доказывать пришлось через `health_events.jsonl` на
> production. Отсюда и строка «ниже порога».
