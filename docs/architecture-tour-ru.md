...
На последней границе `PymaxMediaGateway` обычная `http(s)`-ссылка из Telegram
рендерится в явный Markdown-синтаксис ссылки PyMax. Так PyMax создаёт нативную
кликабельную сущность, но видимый URL и текст, по которому bridge сопоставляет
исходящее подтверждение, не меняются. Уже размеченные ссылки и URL, которые
ограниченный formatter PyMax не может представить безопасно, остаются текстом.
