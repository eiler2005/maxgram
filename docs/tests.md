...
| `test_degraded_channel_prod_like_seven_photos_waits_for_raw_refs` | Обезличенный prod-like regression: typed `CHANNEL` с 7 `PHOTO` без refs ждёт поздний raw/history payload с fake refs и доставляет все 7 фото без partial. |
...
| `test_watchdog_command_links_full_clickable_url` | Ссылка на документацию полная: относительный путь в Telegram не кликается. |
